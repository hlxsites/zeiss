...
import { getLocale, getEnvType } from './utils.js';
 ...
 * @returns
 */
export function pageEnvironment(envType) {
  const envName = {
    live: 'prod_publish',
    preview: 'publish',
    dev: 'local_publish',
...
function getContainerId(envType) {
...
function configureGoogleTagManager(url, envType) {
  ...
  const conf = {
    ...
    pageEnvironment: pageEnvironment(envType),
    pageIdentifier: 'main',
    pageLanguage: locale,
